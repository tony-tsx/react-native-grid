...
type TreeDeepMerge<A, B, C> = DeepMerge<A, DeepMerge<B, C>>

namespace Style {

  export const create = <P, OC extends boolean = false>(
    Component: ComponentType<P>,
    useClass?: OC ): OC extends true ? new ( props: P & Props ) => React.Component<P & Props> :
      React.StatelessComponent<P & Props> => {

    if ( useClass ) return ( class extends React.Component<P & Props> {
        public render = () => {
          const { style, props } = Style.parser( this.props )
          return <Component {...props as any} style={style}/>
        }
      } ) as any

    return ( ( props: P ) => {
        const { style, props: _props } = Style.parser( props )
        return <Component {..._props as any} style={style}/>
      } ) as any
  ...
  export namespace Animation {
    ...
    export interface Props extends Centralization, Positions, Spaces, Direction, Flex, Border {
      h?: Resolution
      w?: Resolution
  ...
  export namespace Styles {
    export type Merge = TreeDeepMerge<ViewStyle, TextStyle, ImageStyle>
